...
The code reads continuously the data from DHT22 (temperature and humidity sensor: `dht`), MLX90640 (thermal camera, `mlx`) and daylight (`day`), and
...
```
cloud:31.50,temp:28.80,hum:57.90,day:0
```
...
```
cloud-0.3.0,dht=1,mlx=1,day=1,cloud=31.53,temp=28.90,hum=58.60,mlx_serial=A08AEA6189,mlx_mode=chess,mlx_resolution=18bit,mlx_frame_rate=2Hz
```
It shows current version, the status of the sensors collection and current value of sensors.

`START`: Start data collection. With no params, it starts collection of measure from all sensors. 
An additional param of the name of the sensor will start only that sensor data collection.
